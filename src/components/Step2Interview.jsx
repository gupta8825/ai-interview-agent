...
import { FaMicrophone, FaWindowMinimize } from "react-icons/fa";
...
const Step2Interview = ({ interviewData, onFinish }) => {
  ...
  useEffect(() => {
    const loadVoices = () => {
      ...
      //Try know female voice first
      const femaleVoice = voices.find(
        (v) =>
          v.name.toLowerCase().includes("zira") ||
          v.name.toLowerCase().includes("samatha") ||
  ...
  useEffect(() => {
    ...
    const runIntro = async () => {
      if (isIntroPhase) {
        await `Hi ${userName}, it's great to meet you today. I hope you're feeling confident and ready.`;
        ...
        if (currentIndex === questions.length - 1) {
          await speakText("Alright, this one might be a bit more challegeing");
        }
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to teal-100 flex items-center justify-center p-4 sm:p-6">
      <div className="w-full max-w-350 min-h-[80vh] bg-white rounded-3xl shadow-2xl border border-gray-200 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="w-full lg:w-[35%] bg-white flex flex-col items-center p-6 space-y-6 border-r border-gray-200">
          <div className="w-full mx-w-md bg-white border border-gray-200 rounded-2xl shadow-2xl shadow-md p-6 space-y-5">
            <video
              src={videoSource}
              key={videoSource}
              ref={videoRef}
